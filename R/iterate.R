...
do_sampling <- function(iter, c_samples, r_samples, t_samples, method=c("bootstrap", "permutation")) 
{
  ...
  if (method == "bootstrap") {
    r_samples <- r_samples[sample(seq_len(r_n), r_n, replace=TRUE)]
    t_samples <- t_samples[sample(seq_len(t_n), t_n, replace=TRUE)]
  }
  if (method == "permutation") {
    Sample <- sample(seq_len(c_n), c_n)
    r_samples <- c_samples[Sample[seq_len(r_n)]]
    t_samples <- c_samples[Sample[seq(r_n+1,r_n+t_n)]]
  }
  output <- list()
...
#' @title Compute Differential Connectivity
#' @keywords internal
